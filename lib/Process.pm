...
#
    ...
    foreach my $mod (sort keys %bufrs) {
        ...
        if ($Bgruven{GRUVEN}->{OPTS}{mono}) {
            ...
            if (@gemfls == 3 and ! $Bgruven{GRUVEN}->{OPTS}{forcep} and ! $ens) {
    ...
    foreach my $mod (keys %{$Bgruven{PROCESS}->{STATIONS}{process}}) {
        ...
        STEPIII: foreach my $stnm (@stnms) {
            ...
            unless (open SFCOUT => "<$sffout") {
                $mesg = "Likely problem (1) with $stid ($stnm) - Continue to next station";
                &Utils::modprint(6,9,144,0,1,$mesg);
                next STEPIII;
            }

            unless (open SNDOUT => "<$sndout") {
                $mesg = "Likely problem (2) with $stid ($stnm) - Continue to next station";
                &Utils::modprint(6,9,144,0,1,$mesg);
                next STEPIII;
            }
            ...
            unless (open TMPFIL => "<$tmpfile") {
                &Utils::modprint(6,9,144,0,1,"Likely problem with $stid ($stnm) - Continue to station");
                next STEPIII;
            }
